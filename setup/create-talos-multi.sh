...
docker network connect \
  ...
  ${SHARED_NETWORK_NAME} "${CLUSTER_PREFIX}-manager-controlplane-1"
docker network connect \
  --ip ${SHARED_NETWORK_SUBNET_PREFIX}.$((IP_BASE + 1)) \
  ${SHARED_NETWORK_NAME} "${CLUSTER_PREFIX}-manager-worker-1"
...
for i in $(seq 1 ${CLUSTER_COUNT}); do
  ...
  docker network connect \
    --ip ${SHARED_CP_IP} \
    ${SHARED_NETWORK_NAME} "${CLUSTER_PREFIX}-${i}-controlplane-1"
  docker network connect \
    --ip ${SHARED_WK_IP} \
    ${SHARED_NETWORK_NAME} "${CLUSTER_PREFIX}-${i}-worker-1"
